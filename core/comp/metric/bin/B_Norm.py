...
def computeMapsFromList(predictions, gold_sentences):
    predictionMap = {}
    goldMap = {}
    # gf = open(goldfile, 'r')
...
def b_norm(refs, preds):
    ref_sentence_lst = refs
    ...
    try:
        ...
        mylogger.error('b_norm',
                       f'Zero division encountered and return 0 to rescue. goldMap={goldMap}, predictonMap={predictionMap}')
        return 0.

def b_norm_new(refs, preds):
    ref_sentence_lst = refs
    references = []
    for idx, ref_sentence in enumerate(ref_sentence_lst):
        references.append("{}\t{}".format(idx, ref_sentence))
    ...
    (goldMap, predictionMap) = computeMapsFromList(predictions, references)
    try:
        return bleuFromMaps(goldMap, predictionMap)[0]
    except ZeroDivisionError:
        mylogger.error('b_norm',
        ...
        mylogger.error('b_norm',
                       f'Unknown error: {e}. predictions: {predictions}, references: {references}')
        return 0.
...
if __name__ == '__main__':
    ...
    if len(predictions) == len(ref_sentence_lst) - 1:
        predictions.append("{}\t{}".format(len(predictions), ""))
    (goldMap, predictionMap) = computeMaps(predictions, reference_file)
    print("B-Norm:", bleuFromMaps(goldMap, predictionMap)[0])
    os.remove("tmp_ref.txt")
